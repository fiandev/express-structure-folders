    ...
    /*
     ...
     set_engines () {
        ...
        for (let key of VIEW_ENGINES) {
          if ( !Object.keys(consolidate).includes(key) ) throw new Error(`view engine '${ key }' not found!`);
          if ( !default_engines.includes(key) ) {
            try {
              app.engine(key, consolidate[key]);
            } catch (e) {
              throw new Error(`please run npm install ${ key }, before using this view engine`);
            }
          }
        }
    }
